...
class  CrearDiaLibrePage:
    ...
    def seleccionar_tipo_dia_libre(self, tipo: str = "&nbsp"):
        ...
        """
        assert tipo is not None, "El tipo de día libre ingresado no es válido."
    ...
    def seleccionar_fecha_inicio(self, fecha: str, jornada: str):
        ...
        """
        assert jornada in ("Mañana", "Tarde"), "La jornada no es válida. Debe ser 'Mañana' o 'Tarde'."
    ...
    def seleccionar_revisor(self, revisor: str = "&nbsp"):
        ...
        """
        assert revisor is not None, "El revisor ingresado no es válido."
    ...
    def ingresar_descripcion(self, descripcion: str):
        ...
        """
        assert ((descripcion is not None) and (len(descripcion) > 0)), "La descripción no puede estar vacía."
    ...
    def verificar_guardado(self, nombre_archivo: str):
        ...
        assert len(TOKENS) == 1, "El nombre de archivo ingresado no es válido. Recuerde que no debe ingresar una extensión"
        ...
        expect(self.__pagina.get_by_text(self.__limpiar_html(self.__descripcion))).to_be_visible()
    ...
    def __verificar_dialogo(self, texto: str):
        ...
        """
        assert self.__msg_dialogo != texto, f"El mensaje del diálogo no es el esperado. Se esperaba: {texto} y se obtuvo: {self.__msg_dialogo}"